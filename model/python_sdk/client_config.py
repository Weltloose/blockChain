...
class client_config:
    ...
    # --------------------------------------
    # configure below
    contract_info_file = "model/python_sdk/bin/contract.ini"  # 保存已部署合约信息的文件
    account_keyfile_path = "model/python_sdk/bin/accounts"  # 保存keystore文件的路径，在此路径下,keystore文件以 [name].keystore命名
    account_keyfile = "test.keystore"
    account_password = "1234"  # 实际使用时建议改为复杂密码
    fiscoChainId = 1  # 链ID，和要通信的节点*必须*一致
    groupid = 1  # 群组ID，和要通信的节点*必须*一致，如和其他群组通信，修改这一项，或者设置bcosclient.py里对应的成员变量
    logdir = "model/python_sdk/bin/logs"  # 默认日志输出目录，该目录必须先建立
    ...
    channel_ca = "model/python_sdk/bin/ca.crt"  # 采用channel协议时，需要设置链证书,如采用rpc协议通信，这里可以留空
    channel_node_cert = "model/python_sdk/bin/node.crt"  # 采用channel协议时，需要设置sdk证书,如采用rpc协议通信，这里可以留空
    channel_node_key = "model/python_sdk/bin/node.key"   # 采用channel协议时，需要设置sdk私钥,如采用rpc协议通信，这里可以留空
    # ---------console mode, support user input--------------
    background = True
    # ---------compiler related--------------
    # path of solc compiler
    solc_path = os.environ["HOME"] + "/.py-solc/solc-v0.4.25/bin/solc"
    solcjs_path = "model/python_sdk/solcjs"
